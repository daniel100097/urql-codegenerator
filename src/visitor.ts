...
> {
    constructor(
        ...
        super(schema, fragments, rawConfig, {
            ...
            urqlImportFrom:
                getConfigValue(rawConfig.urqlImportFrom, null) || undefined,
        });
    ...
    public getImports(): string[] {
        ...
        if (this.config.withComponent || this.config.withHooks) {
            imports.push(
                `import * as Urql from '${
                    this.config.urqlImportFrom || 'urql'
                }';`
            );
        }
    ...
    private _buildComponent(
        ...
        const isVariablesRequired =
            ...
            node.variableDefinitions?.some(
                (variableDef) => variableDef.type.kind === Kind.NON_NULL_TYPE
            );
...
`;
    ...
    private _buildHooks(
        ...
        const operationName: string = this.convertName(node.name?.value ?? '', {
            prefix: this.config.omitOperationSuffix
                ? ''
